...
private:
  ...
  const uint16_t seuilDetectionAdversaire = 5;  // Seuil au dela duquel on considère que l'adversaire est détecté (La valeur doit être supérieur ou égale à ce nombre)
...
public:
  ...
  void combattre();
  void lancerStrategie2();
  void lancerStrategie3();
